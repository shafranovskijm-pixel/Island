...
var locations := {
    ...
    "library": {"name":"Библиотека Святого Картографа","center":Vector2(855,515),"radius":120.0,"public":true,"tags":["books","study","knowledge","skills"],"faction":""},
...
var secrets := {"crypt_entrance":false,"occult_order":false,"vampires":false,"castle_passage":false}

func current_location(pos:Vector2) -> String:
    var best := "wilderness";var best_d := INF
    for id in locations.keys():
        var loc:Dictionary=locations[id];var d:=pos.distance_to(loc["center"])
        if d<=float(loc["radius"]) and d<best_d:best=id;best_d=d
    return best
...
func can_enter(id:String,state:Dictionary)->Dictionary:
    ...
    match id:
        "castle":
            if int(state.get("influence",0))>=3 or bool(state.get("royal_invitation",false)) or int(state.get("stealth",0))>=4:return {"ok":true,"reason":""}
        ...
        "crypt":
            if secrets["crypt_entrance"] or int(state.get("occult",0))>=2 or int(state.get("stealth",0))>=3:return {"ok":true,"reason":""}
            return {"ok":false,"reason":"Ты пока не знаешь, как открыть древний вход в склеп."}
        "occult_lodge":
            if secrets["occult_order"] or int(state.get("magic",0))>=3:return {"ok":true,"reason":""}
...
func spawn_points()->Dictionary:
    return {
        "king":Vector2(1060,485),"queen":Vector2(1015,520),"chancellor":Vector2(1110,545),"librarian":Vector2(855,515),
        "captain_guard":Vector2(1230,625),"priest":Vector2(785,280),"undertaker":Vector2(405,330),"vampire":Vector2(315,245),
        "cult_leader":Vector2(525,255),"beggar":Vector2(665,865),"smuggler":Vector2(735,825),"mage":Vector2(1020,225)
    }
