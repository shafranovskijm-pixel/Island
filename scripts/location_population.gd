...
func extra_npcs(points:Dictionary)->Array:
    return [
        ...
        _npc("librarian","Элвин Тихий","библиотекарь",points["librarian"],"","library",Color("#9a8f72"),"собрать знания острова и сохранить редкие книги",26),
...
func _npc(id:String,name:String,role:String,pos:Vector2,faction:String,home_location:String,color:Color,goal:String,influence:int)->Dictionary:
    return {"id":id,"name":name,"role":role,"pos":pos,"color":color,"rel":0,"memory":[],"suspicion":0,"faction":faction,"home_location":home_location,"goal":goal,"influence":influence,"location_id":home_location,"alive":true,"hidden":home_location in ["crypt","occult_lodge"]}
...
func location_dialogue(npc:Dictionary,location_id:String)->Array:
    match str(npc.get("id","")):
        ...
        "librarian":return ["Попросить книгу для обучения","Спросить о редких знаниях","Уйти"]
