...
func _init():
    ...
    var production=RefCounted.new()
    production.set("hunger_pressure",0.0)
    production.set("resources",{"medicine":5.0})
    var life=HealthLifecycle.new()
    npcs=life.tick(npcs,production,2,7.0)
