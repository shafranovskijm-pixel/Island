...
func draw_building(canvas:CanvasItem,pos:Vector2,size:Vector2,label:String,stone:bool=false):
    canvas.draw_rect(Rect2(pos+Vector2(5,7),size),palette["shadow"])
    canvas.draw_rect(Rect2(pos,size),palette["stone"] if stone else palette["wood"])
    ...
    canvas.draw_rect(Rect2(pos+Vector2(size.x*.42,size.y*.55),Vector2(18,size.y*.45)),Color("#3a2d27"))
    canvas.draw_string(ThemeDB.fallback_font,pos+Vector2(5,-27),label,0,size.x,10,Color("#eee5cf"))

func draw_person(canvas:CanvasItem,pos:Vector2,role:String="",vampire:bool=false):
    canvas.draw_ellipse if false else null
    canvas.draw_circle(pos+Vector2(2,5),9,palette["shadow"])
    canvas.draw_rect(Rect2(pos+Vector2(-6,-1),Vector2(12,18)),palette["cloth"])
    canvas.draw_circle(pos+Vector2(0,-7),6,palette["vampire"] if vampire else palette["skin"])
