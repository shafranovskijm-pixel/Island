...
func buy(species:String,owner:String="player")->Dictionary:
    ...
    var d=defs[species];var a={"id":"livestock_%d"%next_id,"species":species,"name":d["name"],"owner":owner,"hunger":10.0,"health":100.0,"bond":15.0,"pregnant":false,"pregnancy_days":0,"product":d["product"],"last_product_day":-1,"mounted":false};next_id+=1;livestock.append(a);return {"ok":true,"animal":a,"price":d["price"]}
func tick(day:int,food_store:float)->float:
    ...
    for a in livestock:
        ...
        if bool(a.get("pregnant",false)):
            ...
            if int(a["pregnancy_days"])>=8:events.append({"type":"livestock_birth","species":a["species"],"text":"В хозяйстве появился детёныш."});a["pregnant"]=false;a["pregnancy_days"]=0
    return food_store
...
func breed(species:String)->Dictionary:
    var candidates=[]
    for a in livestock:
        if str(a.get("species",""))==species and float(a.get("health",0))>60:candidates.append(a)
    if candidates.size()<2:return {"ok":false,"reason":"Нужны как минимум два здоровых животных одного вида."}
    candidates[0]["pregnant"]=true;candidates[0]["pregnancy_days"]=0;return {"ok":true}
